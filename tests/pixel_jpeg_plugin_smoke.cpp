...
int main(int argc, char** argv) {
  ...
  const auto encoder_configure_ec = encoder_api.configure(
      encoder_ctx.context, PIXEL_CODEC_PROFILE_JPEG_LOSSLESS, nullptr);
  ...
  auto first_encode_request = make_encoder_request(
      source, PIXEL_CODEC_PROFILE_JPEG_LOSSLESS, tiny_output, 0);
  ...
  auto second_encode_request = make_encoder_request(
      source, PIXEL_CODEC_PROFILE_JPEG_LOSSLESS, encoded_output, 0);
  ...
  const auto decoder_configure_ec = decoder_api.configure(
      decoder_ctx.context, PIXEL_CODEC_PROFILE_JPEG_LOSSLESS, nullptr);
  ...
  const pixel_option_list lossy_option_list{
      lossy_options, lossy_option_count};

  const auto encoder_configure_lossy_ec = encoder_api.configure(
      encoder_ctx.context, PIXEL_CODEC_PROFILE_JPEG_LOSSY, &lossy_option_list);
  ...
  const auto decoder_configure_lossy_ec = decoder_api.configure(
      decoder_ctx.context, PIXEL_CODEC_PROFILE_JPEG_LOSSY, nullptr);
  ...
  pixel_output_buffer tiny_lossy_output{};
  auto first_lossy_encode_request = make_encoder_request(
      source, PIXEL_CODEC_PROFILE_JPEG_LOSSY, tiny_lossy_output, 0);
  ...
  pixel_output_buffer lossy_output{};
  ...
  auto second_lossy_encode_request = make_encoder_request(
      source, PIXEL_CODEC_PROFILE_JPEG_LOSSY, lossy_output, 0);
