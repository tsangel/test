...
def test_set_pixel_data_native_single_frame_roundtrip():
    ...
    decoded = dicom_file.to_array(frame=0, scaled=False)
...
def test_set_pixel_data_native_multi_frame_roundtrip():
    ...
    decoded = dicom_file.to_array(frame=-1, scaled=False)
    assert decoded.shape == (2, 4, 4)
    assert np.array_equal(decoded, source)
    assert np.array_equal(dicom_file.to_array(frame=0, scaled=False), frame0)
    assert np.array_equal(dicom_file.to_array(frame=1, scaled=False), frame1)
