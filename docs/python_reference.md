...
Read a DICOM file from disk and return a `DicomFile`. Parsing is eager up to `load_until` (default: entire file). When `keep_on_error=True`, partially read data is kept instead of throwing.
