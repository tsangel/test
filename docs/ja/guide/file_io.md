...
このページでは、ディスクとメモリの入力、部分的なロード、およびファイル、バイト、ストリームのメイン出力パスについて説明します。
...
- `write_with_transfer_syntax(...)` は、異なる転送構文を使用してファイルまたはストリームに直接書き込むための出力指向のトランスコード パスです。これは、ピクセル圧縮を `HTJ2KLossless` などに変更するときに必要になることがよくあります。最初にソース オブジェクトを変更しません。 C++ では、同じ API ファミリにもストリーム オーバーロードがあります。
...
- 後のタグへのアクセスでは、暗黙的に解析が続行されません。 C++ と Python の両方で、後でさらに必要になったときに `ensure_loaded(tag)` を呼び出します。 C++ では、`ensure_loaded(...)` は、`"Rows"_tag`、`"(0028,FFFF)"_tag`、`dicom::Tag(0x0028, 0x0010)` などの `Tag` を受け取ります。 Python では、`ensure_loaded(...)` は、`Tag`、パックされた `int`、または単一タグのキーワード文字列を受け入れます。ドット付きのタグパス文字列はサポートされていません。
