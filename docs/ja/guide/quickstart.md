...
```{note}
サーバー上でメタデータ参照、ファイル I/O、トランスコードのワークフローだけが必要なら、
`pip install dicomsdl` だけで十分です。
```

source build、custom wheel、テストのワークフローが必要なら [Build Python From Source](../developer/build_python_from_source.md) を参照してください。
...
`DicomFile` はルート `DataSet` へのアクセスをそのまま転送するので、Python では `df.Rows` や `df.PatientName` のようなふつうのトップレベル keyword 読み取りが、たいてい最短で推奨される metadata read 経路です。
ネストした leaf lookup には `df.get_value("Seq.0.Tag")` を使い、型付き値ではなく `DataElement` メタデータが必要なときは `df["Rows"]` / `df.get_dataelement(...)` を使ってください。
既知の keyword が単に欠落している場合は `None` を返し、不明な keyword は引き続き `AttributeError` を送出します。
...
ネストした sequence を再帰的にたどる必要がある場合は [DataSet Visit and Walk](dataset_visit_and_walk.md) を参照してください。
必要な tag だけを読みたい場合は [Selected Read](selected_read.md) を参照してください。
...
decode option、frame 選択、出力レイアウト制御が必要なら [Pixel Decode](pixel_decode.md) を参照してください。

5. Pillow ですばやく画像をプレビューする
...
再帰的な走査やサブツリーのスキップが必要な場合は [DataSet Visit and Walk](dataset_visit_and_walk.md) を参照してください。
ディスクやメモリから必要な tag だけを読みたい場合は [Selected Read](selected_read.md) を参照してください。
