...
`None` は、解決された VR の適切な長さ 0 表現の単なる短縮形です。
明示的な空のペイロードを使用して同じインテントを記述することもできます。
...
- `None` -> 現在の長さゼロの要素を保持または作成します
- 空のペイロード (`""`、`[]`、`b""`) -> 要素は `length == 0` で存在し続けます。
...
この形式はロールバック セマンティクスを提供しません。 `False` が返された場合、データセット
有効なままですが、宛先要素の状態は指定されていません。
...
属性代入は、標準キーワードベース更新に対する値指向の簡便な糖衣構文のままです。
これは簡潔で、`DataSet` アクセスと転送された `DicomFile` アクセスの両方で機能します。
`set_value(...)` とは異なり、このパスは割り当て失敗時に `False` を返すのではなく発生します。
したがって、通常は、開発、ノートブック、対話型の使用に適しています。
明示的なエラー処理を必要とする製品コード。
...
`len(df)` は、`DicomFile` からルート データセット サイズを転送します。
...
データセット オブジェクトがすでにある場合は、`ds.size()` を使用します。
ファイル オブジェクトから作業している場合は、`len(df)` を使用します。

＃＃＃ ごみ（）

`dump()` は、`DicomFile` と `DataSet` の両方について、人間が判読できるタブ区切りのダンプ文字列を返します。
...
- ファイルにバックアップされたルート データセットでは、`dump()` は、ダンプをフォーマットする前に、未読の残りの要素もロードします。
