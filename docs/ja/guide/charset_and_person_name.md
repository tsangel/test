...
トランスコードには、対象をエンコードする前にソースをデコードする段階もあります。現在のデータセットに、現在の宣言ではデコードできないバイトがすでに含まれている場合にも、同じポリシー名が適用されます。

たとえば、現在の宣言が `ISO_IR 192` であるが、保存された生のテキスト値に無効な UTF-8 バイト `b"\xFF"` が含まれている場合、ソース デコード ステージは次のように分岐します。

|比較ポイント | `strict` | `replace_qmark` | `replace_unicode_escape` |
| --- | --- | --- | --- |
|現在保存されているバイトがすでにデコード不能な場合 | `set_specific_charset()` は例外を送出して中断します。 |トランスコードは続行され、デコードできないソース バイト列は `?` に置き換えられます。 |トランスコードは続行され、可視のバイト エスケープに置き換えられます。 |
|生のバイト `b"\xFF"` の置換例 |呼び出しが失敗するため、トランスコード後のテキストは生成されません。 | `?` | `(0xFF)` |
|ターゲット エンコード時のフォールバックと異なる理由 | Unicode テキストを復元できなかったため、トランスコードを続行できません。 |Unicode コード ポイントを復元できなかったため、フォールバックは `?` のみです。 |Unicode コード ポイントを復元できなかったため、フォールバックは `(U+XXXX)` ではなく `(0xNN)` のバイト エスケープになります。 |
