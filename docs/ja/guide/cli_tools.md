...
これで `dicomdump`、`dicomconv`、`dicomshow` のコンソールスクリプトが入ります。
...
独立した C++ 用 `dicomshow` 実行ファイルはありません。`dicomshow` は Python の
...
エラーは `dicomdump:`, `dicomshow:`, `dicomconv:` のような接頭辞付きで標準エラーへ出力されます。
