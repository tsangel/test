...
エンコードするネイティブの保存ピクセルがすでにある場合は、`set_pixel_data()` を使用します。現在の `DicomFile` に既にピクセル データがあり、それをメモリ内でトランスコードする場合は、`set_transfer_syntax()` を使用します。最初にソース オブジェクトを変更せずに、ゴールが別の転送構文で出力される場合は、`write_with_transfer_syntax()` を使用します。同じ転送構文とオプション セットが複数の呼び出しで再利用される場合、またはより長いエンコード ループを開始する前にその構成を検証する必要がある場合は、`EncoderContext` を作成します。
...
- `set_pixel_data(...)`
- `pixel::ConstPixelSpan` で明示的にレイアウトを記述したネイティブ ソース バッファからのピクセル データを置き換えます。
- `create_encoder_context(...)` + `set_pixel_data(...)` / `set_transfer_syntax(...)`
- 繰り返されるエンコードまたはトランスコード ループの外側に設定された 1 つの転送構文とオプション セットを保持します。
...
- `create_encoder_context(...)` + `set_pixel_data(...)` / `set_transfer_syntax(...)`
- 1 つの Python `options` オブジェクトを事前に解析して検証し、結果のコンテキストを繰り返しの呼び出しで再利用します。
- `write_with_transfer_syntax(...)`
- 最初にソース オブジェクトを変更せずに、別の転送構文をファイルに直接書き込みます。
- `set_transfer_syntax(...)`
- 後で同じオブジェクトからの読み取りまたは書き込みで新しい構文を使用する場合は、メモリ内の現在の `DicomFile` をトランスコードします。
