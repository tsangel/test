...
실패가 아니라 물리 extent가 겹치고 spacing/orientation/grid mapping 때문에
interpolation 또는 resampling이 필요한 상태다. grid는 같고 extent만 다른
경우는 `different_extent`로 보고하며, caller가 crop/pad/clip으로 처리할 수
있고 resampling은 필요하지 않다.
...
`SliceStackOptions`에서 일반 geometry tolerance는 `tolerance`에 두지만,
duplicate slice position 판정은 `slice_position_tolerance_mm`, in-plane origin
drift 판정은 `origin_residual_tolerance_mm`를 사용한다. duplicate position은
기본적으로 실패이며, `allow_duplicate_positions`는 analysis를 계속하게 할 뿐
zero-gap stack을 uniform volume plan으로 만들지는 않는다.
...
NM Image Storage는 generic enhanced helper가 아니라 `analyze_nm_frame_stack()`과
`plan_nm_frame_stack()`을 사용한다. 현재 MVP adapter는 범위를 좁게 잡는다.
`ImageType` Value 3가 `RECON TOMO` 또는 `RECON GATED TOMO`이고,
`FrameIncrementPointer`가 정확히 `SliceVector` 하나만 가져야 한다. `SliceVector`는 root
image plane normal과 `SpacingBetweenSlices`, 없으면 `SliceThickness`를 이용해
frame별 plane origin으로 확장된다. Projection 성격의 `TOMO` / `GATED TOMO`와
추가 vector가 섞인 NM organization은 regular reconstructed slice stack으로 취급하지 않고
거부한다.
...
- NM Image Storage는 NM-specific frame organization을 가지므로 generic enhanced
  stack으로 취급하지 않는다. 현재는 reconstructed TOMO `SliceVector` adapter만
  구현되어 있다.
