...
예를 들어 소스 텍스트가 `홍길동`이고 대상 문자 집합이 `ISO_IR 100`인 경우 해당 대상 문자 집합은 한국어 문자를 직접 나타낼 수 없습니다. 그러면 정책은 다음과 같이 다양해집니다.

| 비교점 | `strict` | `replace_qmark` | `replace_unicode_escape` |
| --- | --- | --- | --- |
| 일부 텍스트를 표현할 수 없는 경우 | `set_specific_charset()`는 던지거나 올리고 멈춥니다. | 트랜스코드가 성공하여 `?`를 대체합니다. | 트랜스코드가 성공하고 눈에 보이는 `(U+XXXX)` 텍스트로 대체됩니다. |
| `홍길동 -> ISO_IR 100`에 대한 결과 예 | 호출이 실패했기 때문에 트랜스코딩된 텍스트가 생성되지 않습니다. | `???` | `(U+D64D)(U+AE38)(U+B3D9)` |
| 데이터세트 커밋 | 변화가 없습니다. | 문자 세트가 업데이트되고 텍스트 VR이 `?`로 다시 작성됩니다. | 문자 세트가 업데이트되고 텍스트 VR이 `(U+XXXX)` 마커로 다시 작성됩니다. |
| `replaced` 출력 | 호출이 실패했기 때문에 적용할 수 없습니다. | 하나 이상의 대체가 발생하는 경우 `true`입니다. | 하나 이상의 대체가 발생하는 경우 `true`입니다. |
...
또한 트랜스코드에는 대상 인코딩 이전에 소스-디코드 단계가 있습니다. 현재 데이터 세트에 현재 선언에서 디코딩할 수 없는 바이트가 이미 포함되어 있는 경우 동일한 정책 이름이 거기에도 적용됩니다.

예를 들어, 현재 선언이 `ISO_IR 192`이지만 저장된 원시 텍스트 값에 잘못된 UTF-8 바이트 `b"\xFF"`가 포함되어 있는 경우 소스 디코드 단계는 다음과 같이 분기됩니다.

| 비교점 | `strict` | `replace_qmark` | `replace_unicode_escape` |
| --- | --- | --- | --- |
| 현재 저장된 바이트가 이미 디코딩 불가능한 경우 | `set_specific_charset()`는 던지거나 올리고 멈춥니다. | 트랜스코드는 계속 진행되며 디코딩할 수 없는 소스 바이트 범위를 `?`로 대체합니다. | 트랜스코드는 계속되며 눈에 보이는 바이트 이스케이프를 대체합니다. |
| 원시 바이트 `b"\xFF"`에 대한 대체 예 | 호출이 실패했기 때문에 트랜스코딩된 텍스트가 생성되지 않습니다. | `?` | `(0xFF)` |
| 이것이 대상 인코딩 대체와 다른 이유 | 유니코드 텍스트가 복구되지 않았으므로 트랜스코드를 계속할 수 없습니다. | 유니코드 코드 포인트가 복구되지 않았으므로 대체 코드는 `?`입니다. | 유니코드 코드 포인트가 복구되지 않았으므로 폴백은 `(U+XXXX)`가 아닌 `(0xNN)` 바이트 이스케이프입니다. |
...
예를 들어 데이터셋이 `ISO_IR 100`로 선언되고 입력 텍스트가 `홍길동`라면 선언된 문자셋은 한글을 직접적으로 표현할 수 없다. `from_utf8_view()`는 다음과 같이 분기됩니다.

| 비교점 | `strict` | `replace_qmark` | `replace_unicode_escape` |
| --- | --- | --- | --- |
| 선언된 문자 세트에서 입력 텍스트를 표현할 수 없는 경우 | 호출이 실패하고 새로운 내용이 저장되지 않습니다. | 호출이 성공하고 `?`를 대체합니다. | 호출이 성공하고 눈에 보이는 `(U+XXXX)` 텍스트로 대체됩니다. |
| `홍길동 -> ISO_IR 100`에 대한 저장된 텍스트의 예 | 인코딩된 텍스트가 생성되지 않습니다. | `???` | `(U+D64D)(U+AE38)(U+B3D9)` |
| 반환 값 | `false` | `true` | `true` |
| `replaced` 출력 | 쓰기가 성공하지 못했기 때문에 `false` | 하나 이상의 대체가 발생한 경우 `true` | 하나 이상의 대체가 발생한 경우 `true` |
