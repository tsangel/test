...
`bulk_data="uri"`일 때 bulk element에 사용하는 URI template입니다.

`PixelData (7FE0,0010)`도 `pixel_data_uri_template`를 따로 주지 않으면
이 template를 그대로 사용합니다.
...
사용합니다. 이 override를 주지 않으면 `PixelData`도
`bulk_data_uri_template`를 사용합니다.
