...
- When `ensure_dataelement(...)` needs to create the requested nested path under an existing non-sequence intermediate element, it can reset that intermediate element to `SQ`.
