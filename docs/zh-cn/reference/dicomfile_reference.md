...
- selected read 会始终考虑 root `TransferSyntaxUID` 和 `SpecificCharacterSet`，将 `ReadOptions.load_until` 作为 selection frontier 的上限，并允许 selection tree 中出现 private/unknown tag，以及 `"70531000"` 这样的 explicit tag string。
