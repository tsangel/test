...
cmake -S "%ROOT_DIR%" -B "%BUILD_DIR%" -DBUILD_TESTING=%BUILD_TESTING% -DDICOM_BUILD_EXAMPLES=%DICOM_BUILD_EXAMPLES% -DDICOMSDL_MSVC_ENABLE_LTCG=%DICOMSDL_MSVC_ENABLE_LTCG% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -G "%GENERATOR%" %TOOLCHAIN_CMAKE_ARGS% %PIXEL_V2_CMAKE_ARGS% %CODEC_PIXEL_CMAKE_ARGS% %CMAKE_EXTRA_ARGS%
if errorlevel 1 exit /b %errorlevel%
