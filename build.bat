...
cmake -S "%ROOT_DIR%" -B "%BUILD_DIR%" -DBUILD_TESTING=%BUILD_TESTING% -DDICOM_BUILD_EXAMPLES=%DICOM_BUILD_EXAMPLES% -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -G "%GENERATOR%" %TOOLCHAIN_CMAKE_ARGS% %CODEC_CMAKE_ARGS% %CODEC_PLUGIN_CMAKE_ARGS% %PIXEL_V2_CMAKE_ARGS% %CMAKE_EXTRA_ARGS%
if errorlevel 1 exit /b %errorlevel%
...
if /I "%MODE_VALUE%"=="builtin" (
	...
	if /I "%MODE_VALUE%"=="shared" (
		...
		rem DICOMSDL_CODEC_*_SHARED uses a removed plugin ABI path.
		rem Keep legacy shared flag OFF and enable pixel v2 shared plugins instead.
		...
		if /I "%CODEC_NAME%"=="JPEG" (
			set "CODEC_PLUGIN_CMAKE_ARGS=%CODEC_PLUGIN_CMAKE_ARGS% -DDICOMSDL_PIXEL_JPEG_PLUGIN=ON"
		) else if /I "%CODEC_NAME%"=="JPEGLS" (
			set "CODEC_PLUGIN_CMAKE_ARGS=%CODEC_PLUGIN_CMAKE_ARGS% -DDICOMSDL_PIXEL_JPEGLS_PLUGIN=ON"
		) else if /I "%CODEC_NAME%"=="JPEG2K" (
			set "CODEC_PLUGIN_CMAKE_ARGS=%CODEC_PLUGIN_CMAKE_ARGS% -DDICOMSDL_PIXEL_OPENJPEG_PLUGIN=ON"
		) else if /I "%CODEC_NAME%"=="HTJ2K" (
			set "CODEC_PLUGIN_CMAKE_ARGS=%CODEC_PLUGIN_CMAKE_ARGS% -DDICOMSDL_PIXEL_HTJ2K_PLUGIN=ON"
		) else if /I "%CODEC_NAME%"=="JPEGXL" (
			set "CODEC_PLUGIN_CMAKE_ARGS=%CODEC_PLUGIN_CMAKE_ARGS% -DDICOMSDL_PIXEL_JPEGXL_PLUGIN=ON"
		)
	) else (
