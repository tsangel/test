...
if not defined DICOMSDL_MSVC_ENABLE_LTCG set "DICOMSDL_MSVC_ENABLE_LTCG=ON"
if not defined DICOMSDL_MSVC_PGO set "DICOMSDL_MSVC_PGO=OFF"
if not defined DICOMSDL_MSVC_PGO_DIR set "DICOMSDL_MSVC_PGO_DIR=%ROOT_DIR%\build-pgo\msvc"
...
set "DICOMSDL_MSVC_PGO=%DICOMSDL_MSVC_PGO:"=%"
set "DICOMSDL_MSVC_PGO_DIR=%DICOMSDL_MSVC_PGO_DIR:"=%"
...
if not defined DICOMSDL_PIXEL_DEFAULT_MODE set "DICOMSDL_PIXEL_DEFAULT_MODE=builtin"
if /I "%DICOMSDL_PIXEL_DEFAULT_MODE%"=="builtin" (
	set "DICOMSDL_PIXEL_DEFAULT_MODE=builtin"
) else (
	if /I "%DICOMSDL_PIXEL_DEFAULT_MODE%"=="shared" (
		set "DICOMSDL_PIXEL_DEFAULT_MODE=shared"
	) else (
		if /I "%DICOMSDL_PIXEL_DEFAULT_MODE%"=="none" (
			set "DICOMSDL_PIXEL_DEFAULT_MODE=none"
		) else (
...
if not defined DICOMSDL_PIXEL_JPEGXL_MODE set "DICOMSDL_PIXEL_JPEGXL_MODE=none"

set "PIXEL_V2_CMAKE_ARGS=-DDICOMSDL_PIXEL_CORE=ON -DDICOMSDL_PIXEL_RUNTIME=ON -DDICOMSDL_PIXEL_RLE_STATIC_PLUGIN=ON"
set "CODEC_PIXEL_CMAKE_ARGS="
set "JPEGXL_MODE_REQUESTED=0"
...
if errorlevel 1 exit /b %errorlevel%
if "%JPEGXL_MODE_REQUESTED%"=="1" set "CODEC_PIXEL_CMAKE_ARGS=%CODEC_PIXEL_CMAKE_ARGS% -DDICOMSDL_ENABLE_JPEGXL=ON"
...
cmake -S "%ROOT_DIR%" -B "%BUILD_DIR%" -DBUILD_TESTING=%BUILD_TESTING% -DDICOM_BUILD_EXAMPLES=%DICOM_BUILD_EXAMPLES% -DDICOMSDL_MSVC_ENABLE_LTCG=%DICOMSDL_MSVC_ENABLE_LTCG% -DDICOMSDL_MSVC_PGO=%DICOMSDL_MSVC_PGO% -DDICOMSDL_MSVC_PGO_DIR="%DICOMSDL_MSVC_PGO_DIR%" -DCMAKE_BUILD_TYPE=%BUILD_TYPE% -G "%GENERATOR%" %TOOLCHAIN_CMAKE_ARGS% %PIXEL_V2_CMAKE_ARGS% %CODEC_PIXEL_CMAKE_ARGS% %CMAKE_EXTRA_ARGS%
if errorlevel 1 exit /b %errorlevel%
...
:resolve_codec_mode
set "CODEC_NAME=%~1"
set "MODE_VAR_NAME=DICOMSDL_PIXEL_%CODEC_NAME%_MODE"
call set "MODE_VALUE=%%%MODE_VAR_NAME%%%"
if not defined MODE_VALUE set "MODE_VALUE=%DICOMSDL_PIXEL_DEFAULT_MODE%"

set "STATIC_OPT_NAME="
set "SHARED_OPT_NAME="
if /I "%CODEC_NAME%"=="JPEG" (
	set "STATIC_OPT_NAME=DICOMSDL_PIXEL_JPEG_STATIC_PLUGIN"
	set "SHARED_OPT_NAME=DICOMSDL_PIXEL_JPEG_PLUGIN"
) else if /I "%CODEC_NAME%"=="JPEGLS" (
	set "STATIC_OPT_NAME=DICOMSDL_PIXEL_JPEGLS_STATIC_PLUGIN"
	set "SHARED_OPT_NAME=DICOMSDL_PIXEL_JPEGLS_PLUGIN"
) else if /I "%CODEC_NAME%"=="JPEG2K" (
	set "STATIC_OPT_NAME=DICOMSDL_PIXEL_OPENJPEG_STATIC_PLUGIN"
	set "SHARED_OPT_NAME=DICOMSDL_PIXEL_OPENJPEG_PLUGIN"
) else if /I "%CODEC_NAME%"=="HTJ2K" (
	set "STATIC_OPT_NAME=DICOMSDL_PIXEL_HTJ2K_STATIC_PLUGIN"
	set "SHARED_OPT_NAME=DICOMSDL_PIXEL_HTJ2K_PLUGIN"
) else if /I "%CODEC_NAME%"=="JPEGXL" (
	set "STATIC_OPT_NAME=DICOMSDL_PIXEL_JPEGXL_STATIC_PLUGIN"
	set "SHARED_OPT_NAME=DICOMSDL_PIXEL_JPEGXL_PLUGIN"
) else (
	echo Error: unsupported codec key %CODEC_NAME%.>&2
	exit /b 1
)

if /I "%MODE_VALUE%"=="builtin" (
	set "MODE_VALUE=builtin"
	set "CODEC_PIXEL_CMAKE_ARGS=%CODEC_PIXEL_CMAKE_ARGS% -D%STATIC_OPT_NAME%=ON -D%SHARED_OPT_NAME%=OFF"
	if /I "%CODEC_NAME%"=="JPEGXL" set "JPEGXL_MODE_REQUESTED=1"
) else (
	if /I "%MODE_VALUE%"=="shared" (
		set "MODE_VALUE=shared"
		set "CODEC_PIXEL_CMAKE_ARGS=%CODEC_PIXEL_CMAKE_ARGS% -D%STATIC_OPT_NAME%=OFF -D%SHARED_OPT_NAME%=ON"
		if /I "%CODEC_NAME%"=="JPEGXL" set "JPEGXL_MODE_REQUESTED=1"
	) else (
		if /I "%MODE_VALUE%"=="none" (
			set "MODE_VALUE=none"
			set "CODEC_PIXEL_CMAKE_ARGS=%CODEC_PIXEL_CMAKE_ARGS% -D%STATIC_OPT_NAME%=OFF -D%SHARED_OPT_NAME%=OFF"
		) else (
