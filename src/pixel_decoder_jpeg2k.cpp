...
OPJ_UINT32 resolve_openjpeg_thread_count(const DicomFile& df, const DecodeOptions& opt) {
	...
	if (static_cast<unsigned long long>(configured_threads) >
		...
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=decoder_threads {} exceeds OpenJPEG limit {}",
...
jpeg2k_frame_source load_jpeg2k_frame_source(const DicomFile& df, std::size_t frame_index) {
	...
	if (!pixel_data || !pixel_data.vr().is_pixel_sequence()) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 requires encapsulated PixelData",
		    df.path());
	}
	...
	if (!pixel_sequence) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 pixel sequence is missing",
		    df.path());
	}
	...
	if (frame_index >= frame_count) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 frame index out of range (frames={})",
	...
	if (!frame) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 frame is missing",
		    df.path(), frame_index);
	}
	...
	if (fragments.empty()) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 frame has no fragments",
		    df.path(), frame_index);
	}
	for (const auto& fragment : fragments) {
		if (fragment.length == 0) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 zero-length fragment is not supported",
			    df.path(), frame_index);
		}
	}
	...
	if (!stream) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 pixel sequence stream is missing",
		    df.path(), frame_index);
	}
	...
	for (const auto& frag : fragments) {
		...
		if (frag.length > std::numeric_limits<std::size_t>::max() - total_size) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 frame size overflow",
			    df.path(), frame_index);
		}
...
opj_image_ptr decode_openjpeg_image(const DicomFile& df, std::size_t frame_index,
	...
	diag::error_and_throw(
	    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 decode failed ({}: {}; {}: {})",
	    df.path(), frame_index,
...
void validate_decoded_image(const DicomFile& df, std::size_t frame_index, const DicomFile::pixel_info_t& info,
	...
	if (decoded_rows != rows || decoded_cols != cols) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 decoded dimensions mismatch (decoded={}x{}, expected={}x{})",
		    df.path(), frame_index, decoded_rows, decoded_cols, rows, cols);
	}
	if (image.numcomps != samples_per_pixel) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 component count mismatch (decoded={}, expected={})",
	...
	if (bytes_per_sample == 0 || bytes_per_sample > 4 || !sv_dtype_is_integral(info.sv_dtype)) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 supports integral sv_dtype up to 32-bit only",
		    df.path(), frame_index);
	}
	...
	if (!expected_signed && max_precision == 32) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 unsigned 32-bit samples are not supported",
		    df.path(), frame_index);
	}
	...
	for (std::size_t c = 0; c < samples_per_pixel; ++c) {
		...
		if (!comp.data) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 component {} has no decoded data",
			    df.path(), frame_index, c);
		}
		if (comp.w != cols || comp.h != rows) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 component {} dimensions mismatch (decoded={}x{}, expected={}x{})",
			    df.path(), frame_index, c, comp.h, comp.w, rows, cols);
		}
		if (comp.sgnd != static_cast<OPJ_UINT32>(expected_signed ? 1 : 0)) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 component {} signedness mismatch (decoded={}, expected={})",
			    df.path(), frame_index, c, comp.sgnd, expected_signed ? 1 : 0);
		}
		if (comp.prec == 0 || comp.prec > max_precision) {
			diag::error_and_throw(
			    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 component {} precision {} exceeds output {} bits",
...
void write_scaled_mono_to_dst(const DicomFile& df, const opj_image_t& image, std::span<std::uint8_t> dst,
	...
	if (!src) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 decoded component has no data",
		    df.path());
	}
...
void decode_jpeg2k_into(const DicomFile& df, const DicomFile::pixel_info_t& info,
	...
	if (!info.ts.is_jpeg2000()) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=transfer syntax is not JPEG2000 ({})",
	...
	if (samples_per_pixel_value != 1 && samples_per_pixel_value != 3 &&
		...
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=only SamplesPerPixel=1/3/4 is supported in current JPEG2000 path",
		    df.path());
	}
	...
	if (!sv_dtype_is_integral(info.sv_dtype)) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 supports integral sv_dtype only",
		    df.path());
	}
	...
	if (src_bytes_per_sample == 0 || src_bytes_per_sample > 4) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 supports integral sv_dtype up to 32-bit only",
		    df.path());
	}
	...
	if (frame_source.total_size == 0) {
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} frame={} reason=JPEG2000 frame has empty codestream",
		    df.path(), frame_index);
	}
	...
	default:
		diag::error_and_throw(
		    "pixel::decode_frame_into file={} reason=JPEG2000 output does not support sv_dtype={}",
		    df.path(), static_cast<int>(info.sv_dtype));
		return;
	}
}
