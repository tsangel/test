...
int jpeg_decoder_decode_frame(void* ctx, const dicomsdl_decoder_request_v1* request,
  ...
  if (!is_jpeg_transfer_syntax_code(request->frame.transfer_syntax_code)) {
  ...
  if (!decode_jpeg_into(info, value_transform, destination, strides, options,
          decode_error, source)) {
  ...
  if (options) {
    for (std::uint32_t i = 0; i < options->count; ++i) {
      ...
      if (!item.key || !item.value) {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
            DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
            "JPEG option key/value is null");
        return 0;
      }
      const std::string_view key(item.key);
      if (!option_key_matches(key, "quality")) {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
            DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
            std::string("unknown JPEG option key: ") + std::string(key));
        return 0;
      }
      if (quality_seen) {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
      ...
      if (!parse_strict_int_option(std::string_view(item.value), quality)) {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
            DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
...
int jpeg_encoder_encode_frame(void* ctx, const dicomsdl_encoder_request_v1* request,
  ...
  const bool lossless = transfer_syntax->is_jpeg_lossless();
