...
#include "pixel_/plugin_abi/builtin/rle_builtin_plugin.hpp"
...
[[nodiscard]] bool is_transfer_syntax_direct_decode_candidate(
	...
	return transfer_syntax.is_uncompressed() || transfer_syntax.is_rle() ||
...
[[nodiscard]] bool try_dispatch_decode_frame_with_direct(const DicomFile& df,
	...
	if (info.ts.is_uncompressed()) {
		...
		decoded = decode_frame_plugin_rle_via_abi(decode_input, decode_error);
