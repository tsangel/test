...
constexpr pixel_supported_profile_flags_v2 kSupportedProfileFlags =
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_DEC_V2 |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSY_DEC_V2 |
    PIXEL_CODEC_PROFILE_JPEGXL_JPEG_RECOMPRESSION_DEC_V2 |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_ENC_V2 |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSY_ENC_V2;
...
pixel_error_code_v2 encoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  c->distance =
      (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_V2) ? 0.0 : 1.0;
  c->effort = 7;
  c->threads = -1;
  ...
  if (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_V2 &&
      c->distance != 0.0) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
