...
pixel_error_code_v2 set_turbojpeg_param(
...
pixel_error_code_v2 encoder_encode_frame(
  ...
  try {
    ...
    {
      const pixel_error_code_v2 ec = set_turbojpeg_param(
    ...
    mutable_request->output.encoded_size = encoded_size;

    if (request->output.encoded_buffer.data == nullptr ||
        request->output.encoded_buffer.size < encoded_size) {
      return fail_detail(c, PIXEL_CODEC_ERR_OUTPUT_TOO_SMALL, "encode_frame",
          "output buffer too small");
    }

    std::memcpy(request->output.encoded_buffer.data, encoded_buffer.get(), encoded_size);
