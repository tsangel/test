#include "pixel_/plugin_abi/builtin/jpegls_builtin_plugin.hpp"
...
using plugin_common::parse_jpegls_near_lossless_configure_option;
...
using plugin_common::validate_jpegls_near_lossless_configure_option;
...
int jpegls_decoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
  ...
  if (!validate_configure_no_options(options, transfer_syntax_code,
          ...
          is_jpegls_transfer_syntax_code, error)) {
    return 0;
  }
...
int jpegls_decoder_decode_frame(void* ctx,
  ...
  if (!prepare_plugin_decode_request(*request, context->transfer_syntax_code, -1,
          ...
          is_jpegls_transfer_syntax_code, prepared, error)) {
    return 0;
  }

  CodecError decode_error{};
  prepared.options.decode_mct = request->frame.decode_mct != 0;
  if (!decode_jpegls_into(prepared.info, prepared.value_transform,
...
int jpegls_encoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
  ...
  int near_lossless_error = kDefaultNearLosslessError;
  if (!parse_jpegls_near_lossless_configure_option(
          options, kDefaultNearLosslessError, near_lossless_error, error)) {
    return 0;
  }
  ...
  if (!validate_jpegls_near_lossless_configure_option(near_lossless_error,
...
int jpegls_encoder_encode_frame(void* ctx,
  ...
  if (!try_encode_jpegls_frame(prepared.source, prepared.rows, prepared.cols,
          prepared.samples_per_pixel, prepared.bytes_per_sample,
          request->frame.bits_allocated, request->frame.bits_stored,
          prepared.source_planar, prepared.row_stride, context->near_lossless_error,
