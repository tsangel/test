#include "pixel_/plugin_abi/builtin/jpegxl_builtin_plugin.hpp"
...
using plugin_common::parse_jpegxl_encoder_configure_options;
...
using plugin_common::validate_jpegxl_encoder_configure_options;
using plugin_common::write_plugin_encoded_output;

[[nodiscard]] bool is_jpegxl_transfer_syntax_code(
    std::uint16_t transfer_syntax_code) noexcept {
  const auto transfer_syntax =
      abi::from_transfer_syntax_code(transfer_syntax_code);
  return transfer_syntax.has_value() && transfer_syntax->is_jpegxl();
}

void* jpegxl_decoder_create() noexcept {
  return new (std::nothrow) JpegXlDecoderPluginContext{};
}

void jpegxl_decoder_destroy(void* ctx) noexcept {
  delete static_cast<JpegXlDecoderPluginContext*>(ctx);
}

int jpegxl_decoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
    const dicomsdl_codec_option_list_v1* options,
    dicomsdl_codec_error_v1* error) noexcept {
  auto* context = static_cast<JpegXlDecoderPluginContext*>(ctx);
  if (!context) {
  ...
  if (!validate_configure_no_options(options, transfer_syntax_code,
          "JPEG-XL decoder does not accept configure options",
          "transfer syntax is not a JPEG-XL syntax",
          is_jpegxl_transfer_syntax_code, error)) {
    return 0;
  }
...
int jpegxl_decoder_decode_frame(void* ctx,
  ...
  if (!prepare_plugin_decode_request(*request, context->transfer_syntax_code, -1,
          "request transfer syntax is not a JPEG-XL syntax",
          "JPEG-XL decoder plugin supports destination dtype == source dtype or f32",
          is_jpegxl_transfer_syntax_code, prepared, error)) {
    return 0;
  }

  CodecError decode_error{};
  prepared.options.decode_mct = request->frame.decode_mct != 0;
  if (!decode_jpegxl_into(prepared.info, prepared.value_transform,
    ...
    set_abi_error_from_codec_error(error, decode_error, "decode_frame",
        "JPEG-XL decoder backend failed");
    return 0;
  }
...
void* jpegxl_encoder_create() noexcept {
  return new (std::nothrow) JpegXlEncoderPluginContext{};
}

void jpegxl_encoder_destroy(void* ctx) noexcept {
  delete static_cast<JpegXlEncoderPluginContext*>(ctx);
}

int jpegxl_encoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
    const dicomsdl_codec_option_list_v1* options,
    dicomsdl_codec_error_v1* error) noexcept {
  auto* context = static_cast<JpegXlEncoderPluginContext*>(ctx);
  if (!context) {
    set_abi_error(error, DICOMSDL_CODEC_INTERNAL_ERROR,
        DICOMSDL_CODEC_STAGE_PLUGIN_LOOKUP, "encoder context is null");
    return 0;
  }
  if (!is_jpegxl_transfer_syntax_code(transfer_syntax_code)) {
  ...
  if (!transfer_syntax->supports_pixel_encode()) {
    set_abi_error(error, DICOMSDL_CODEC_UNSUPPORTED,
        DICOMSDL_CODEC_STAGE_VALIDATE,
        "transfer syntax is decode-only for JPEG-XL encoder");
    return 0;
  }

  JpegXlOptions configured_options{};
  if (transfer_syntax->is_lossless()) {
    configured_options.distance = 0.0;
  }
  if (!parse_jpegxl_encoder_configure_options(
          options, configured_options, error)) {
    return 0;
  }
  if (!validate_jpegxl_encoder_configure_options(configured_options,
          transfer_syntax->is_lossless(), transfer_syntax->is_lossy(), error)) {
    return 0;
  }
...
int jpegxl_encoder_encode_frame(void* ctx,
  ...
  if (!try_encode_jpegxl_frame(prepared.source, prepared.rows, prepared.cols,
          ...
          prepared.row_stride, prepared.lossless, context->options,
          encoded, encode_error)) {
    set_abi_error_from_codec_error(error, encode_error, "encode_frame",
        "JPEG-XL encoder backend failed");
    return 0;
  }
...
jpegxl_decoder_plugin_api_v1() noexcept {
  static const auto api = [] {
    ...
    value.info.plugin_key = "jpegxl";
    value.info.display_name = "JPEG-XL Builtin ABI Decoder";
    value.create = &jpegxl_decoder_create;
    value.destroy = &jpegxl_decoder_destroy;
    value.configure = &jpegxl_decoder_configure;
    value.decode_frame = &jpegxl_decoder_decode_frame;
    return value;
  }();
  return api;
}
...
jpegxl_encoder_plugin_api_v1() noexcept {
  static const auto api = [] {
    ...
    value.info.plugin_key = "jpegxl";
    value.info.display_name = "JPEG-XL Builtin ABI Encoder";
    value.create = &jpegxl_encoder_create;
    value.destroy = &jpegxl_encoder_destroy;
    value.configure = &jpegxl_encoder_configure;
    value.encode_frame = &jpegxl_encoder_encode_frame;
    return value;
  }();
  return api;
}

bool invoke_jpegxl_decoder_via_plugin_api(
  ...
  const auto& api = jpegxl_decoder_plugin_api_v1();
  ...
  if (guard.context == nullptr) {
    set_codec_error(out_error, CodecStatusCode::internal_error,
        "plugin_lookup", "JPEG-XL decoder plugin create() returned null context");
    return false;
  }
  ...
  if (!api.configure(guard.context, request.frame.transfer_syntax_code,
    ...
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "parse_options", "JPEG-XL decoder configure failed");
  ...
  if (!api.decode_frame(guard.context, &request, &plugin_error)) {
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "decode_frame", "JPEG-XL decoder failed");
    if (out_error.code == CodecStatusCode::ok) {
      out_error.code = CodecStatusCode::backend_error;
    }
    return false;
...
bool invoke_jpegxl_encoder_via_plugin_api(const CodecEncodeFrameInput& input,
  ...
  if (!api.configure(
    ...
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "parse_options", "JPEG-XL encoder configure failed");
    if (out_error.code == CodecStatusCode::ok) {
      out_error.code = CodecStatusCode::backend_error;
    }
    return false;
  ...
  for (int attempt = 0; attempt < 4; ++attempt) {
    ...
    if (api.encode_frame(guard.context, &request, &plugin_error)) {
      if (request.output.encoded_size >
        ...
        set_codec_error(out_error, CodecStatusCode::internal_error,
            "encode_frame",
            "JPEG-XL encoder returned encoded_size larger than output buffer");
        out_encoded_frame.clear();
        return false;
      }
    ...
    if (plugin_error.status_code != DICOMSDL_CODEC_OUTPUT_TOO_SMALL) {
      out_error = abi::decode_plugin_error_v1(
          plugin_error, "encode_frame", "JPEG-XL encoder failed");
      if (out_error.code == CodecStatusCode::ok) {
        out_error.code = CodecStatusCode::backend_error;
      }
      out_encoded_frame.clear();
  ...
  set_codec_error(out_error, CodecStatusCode::backend_error, "encode_frame",
      "JPEG-XL encoder repeatedly returned OUTPUT_TOO_SMALL");
  out_encoded_frame.clear();
  return false;
}
...
bool decode_frame_plugin_jpegxl_via_abi(
  ...
  try {
    ...
    return invoke_jpegxl_decoder_via_plugin_api(input, out_error);
...
bool encode_frame_plugin_jpegxl_via_abi(const CodecEncodeFrameInput& input,
    ...
    CodecError& out_error) noexcept {
  try {
    return invoke_jpegxl_encoder_via_plugin_api(
