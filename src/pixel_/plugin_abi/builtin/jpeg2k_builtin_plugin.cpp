#include "pixel_/plugin_abi/builtin/jpeg2k_builtin_plugin.hpp"
...
using plugin_common::OptionParseResult;
using plugin_common::parse_decoder_threads_option;
...
using plugin_common::validate_decoder_threads_option;
using plugin_common::validate_j2k_like_encoder_configure_options;
using plugin_common::write_plugin_encoded_output;

[[nodiscard]] bool is_jpeg2k_transfer_syntax_code(
    std::uint16_t transfer_syntax_code) noexcept {
  const auto transfer_syntax =
      abi::from_transfer_syntax_code(transfer_syntax_code);
  return transfer_syntax.has_value() && transfer_syntax->is_jpeg2000() &&
         !transfer_syntax->is_htj2k();
}

void* jpeg2k_decoder_create() noexcept {
  return new (std::nothrow) Jpeg2kDecoderPluginContext{};
}

void jpeg2k_decoder_destroy(void* ctx) noexcept {
  delete static_cast<Jpeg2kDecoderPluginContext*>(ctx);
}

int jpeg2k_decoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
    const dicomsdl_codec_option_list_v1* options,
    dicomsdl_codec_error_v1* error) noexcept {
  auto* context = static_cast<Jpeg2kDecoderPluginContext*>(ctx);
  if (!context) {
    set_abi_error(error, DICOMSDL_CODEC_INTERNAL_ERROR,
        DICOMSDL_CODEC_STAGE_PLUGIN_LOOKUP, "decoder context is null");
    return 0;
  }
  if (!is_jpeg2k_transfer_syntax_code(transfer_syntax_code)) {
    set_abi_error(error, DICOMSDL_CODEC_UNSUPPORTED,
        DICOMSDL_CODEC_STAGE_VALIDATE,
        "transfer syntax is not a JPEG2000 syntax");
    return 0;
  }
    ...
    for (std::uint32_t i = 0; i < options->count; ++i) {
      ...
      if (!item.key || !item.value) {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
            ...
            "decoder option key/value is null");
        return 0;
      }
      const std::string_view key(item.key);
      switch (parse_decoder_threads_option(key, std::string_view(item.value),
          true, threads_seen, configured_threads,
          "duplicate JPEG2000 decoder option key: threads",
          "JPEG2000 decoder threads option must be an integer", error)) {
      ...
      set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
          DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
          "unknown JPEG2000 decoder option key");
      return 0;
    }
  }
  if (!validate_decoder_threads_option(configured_threads,
...
int jpeg2k_decoder_decode_frame(void* ctx,
  ...
  if (!prepare_plugin_decode_request(*request, context->transfer_syntax_code,
          context->decoder_threads,
          "request transfer syntax is not a JPEG2000 syntax",
          "JPEG2000 decoder plugin supports destination dtype == source dtype or f32",
          is_jpeg2k_transfer_syntax_code, prepared, error)) {
    return 0;
  }

  CodecError decode_error{};
  if (!decode_jpeg2k_into(prepared.info, prepared.value_transform,
    ...
    set_abi_error_from_codec_error(error, decode_error, "decode_frame",
        "JPEG2000 decoder backend failed");
    return 0;
  }
...
void* jpeg2k_encoder_create() noexcept {
  return new (std::nothrow) Jpeg2kEncoderPluginContext{};
}

void jpeg2k_encoder_destroy(void* ctx) noexcept {
  delete static_cast<Jpeg2kEncoderPluginContext*>(ctx);
}

int jpeg2k_encoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
    const dicomsdl_codec_option_list_v1* options,
    dicomsdl_codec_error_v1* error) noexcept {
  auto* context = static_cast<Jpeg2kEncoderPluginContext*>(ctx);
  if (!context) {
    set_abi_error(error, DICOMSDL_CODEC_INTERNAL_ERROR,
  ...
  if (!parse_j2k_like_encoder_configure_options(options,
          "JPEG2000 option key/value is null",
          "unknown JPEG2000 option key: ", parsed_options, error)) {
    return 0;
  }
  if (!validate_j2k_like_encoder_configure_options(
          parsed_options, "J2kOptions", error)) {
    return 0;
  }

  J2kOptions configured_options{};
...
int jpeg2k_encoder_encode_frame(void* ctx,
  ...
  if (!try_encode_jpeg2k_frame(prepared.source, prepared.rows, prepared.cols,
...
jpeg2k_decoder_plugin_api_v1() noexcept {
  static const auto api = [] {
    ...
    value.info.plugin_key = "jpeg2k";
    value.info.display_name = "JPEG2000 Builtin ABI Decoder";
    value.create = &jpeg2k_decoder_create;
    value.destroy = &jpeg2k_decoder_destroy;
    value.configure = &jpeg2k_decoder_configure;
    value.decode_frame = &jpeg2k_decoder_decode_frame;
    return value;
  }();
  return api;
}
...
jpeg2k_encoder_plugin_api_v1() noexcept {
  static const auto api = [] {
    ...
    value.info.plugin_key = "jpeg2k";
    value.info.display_name = "JPEG2000 Builtin ABI Encoder";
    value.create = &jpeg2k_encoder_create;
    value.destroy = &jpeg2k_encoder_destroy;
    value.configure = &jpeg2k_encoder_configure;
    value.encode_frame = &jpeg2k_encoder_encode_frame;
    return value;
  }();
  return api;
}

bool invoke_jpeg2k_decoder_via_plugin_api(
  ...
  const auto& api = jpeg2k_decoder_plugin_api_v1();
  ...
  if (guard.context == nullptr) {
    set_codec_error(out_error, CodecStatusCode::internal_error,
        "plugin_lookup", "JPEG2000 decoder plugin create() returned null context");
    return false;
  }
  ...
  dicomsdl_codec_option_kv_v1 option_items[1] = {
  ...
  if (!api.configure(guard.context, request.frame.transfer_syntax_code,
    ...
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "parse_options", "JPEG2000 decoder configure failed");
  ...
  if (!api.decode_frame(guard.context, &request, &plugin_error)) {
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "decode_frame", "JPEG2000 decoder failed");
    if (out_error.code == CodecStatusCode::ok) {
      out_error.code = CodecStatusCode::backend_error;
    }
    return false;
...
bool invoke_jpeg2k_encoder_via_plugin_api(const CodecEncodeFrameInput& input,
  ...
  const auto& api = jpeg2k_encoder_plugin_api_v1();
  EncoderContextGuard guard{};
  guard.api = &api;
  guard.context = api.create ? api.create() : nullptr;
  if (guard.context == nullptr) {
    set_codec_error(out_error, CodecStatusCode::internal_error, "plugin_lookup",
        "JPEG2000 encoder plugin create() returned null context");
    return false;
  }
  ...
  if (!api.configure(
    ...
    out_error = abi::decode_plugin_error_v1(
        plugin_error, "parse_options", "JPEG2000 encoder configure failed");
    if (out_error.code == CodecStatusCode::ok) {
      out_error.code = CodecStatusCode::backend_error;
    }
    return false;
  ...
  for (int attempt = 0; attempt < 4; ++attempt) {
    ...
    if (api.encode_frame(guard.context, &request, &plugin_error)) {
      if (request.output.encoded_size >
        ...
        set_codec_error(out_error, CodecStatusCode::internal_error,
            "encode_frame",
            "JPEG2000 encoder returned encoded_size larger than output buffer");
        out_encoded_frame.clear();
        return false;
      }
    ...
    if (plugin_error.status_code != DICOMSDL_CODEC_OUTPUT_TOO_SMALL) {
      out_error = abi::decode_plugin_error_v1(
          plugin_error, "encode_frame", "JPEG2000 encoder failed");
      if (out_error.code == CodecStatusCode::ok) {
        out_error.code = CodecStatusCode::backend_error;
      }
      out_encoded_frame.clear();
  ...
  set_codec_error(out_error, CodecStatusCode::backend_error, "encode_frame",
      "JPEG2000 encoder repeatedly returned OUTPUT_TOO_SMALL");
  out_encoded_frame.clear();
  return false;
}
...
bool decode_frame_plugin_jpeg2k_via_abi(
  ...
  try {
    ...
    return invoke_jpeg2k_decoder_via_plugin_api(input, out_error);
...
bool encode_frame_plugin_jpeg2k_via_abi(const CodecEncodeFrameInput& input,
    ...
    CodecError& out_error) noexcept {
  try {
    return invoke_jpeg2k_encoder_via_plugin_api(
