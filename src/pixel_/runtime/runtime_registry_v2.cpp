...
const PluginRegistryV2* current_registry_v2() noexcept {
  return current_registry();
}

bool register_external_codec_plugin_from_library_v2(
    std::string_view library_path, std::string* out_plugin_key, std::string* out_error) {
  return register_external_codec_plugin_from_library(
      library_path, out_plugin_key, out_error);
}

bool register_external_decoder_plugin_static_v2(
    const pixel_decoder_plugin_api_v2* api, std::string* out_error) {
  return register_external_decoder_plugin_static(api, out_error);
}

bool register_external_encoder_plugin_static_v2(
    const pixel_encoder_plugin_api_v2* api, std::string* out_error) {
  return register_external_encoder_plugin_static(api, out_error);
}

bool unregister_external_codec_plugin_v2(
    std::string_view plugin_key, std::string* out_error) {
  return unregister_external_codec_plugin(plugin_key, out_error);
}
