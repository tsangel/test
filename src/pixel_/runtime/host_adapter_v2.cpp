...
pixel_error_code_v2 configure_host_decoder_context_v2(HostDecoderContextV2* ctx,
  ...
  if (!resolve_codec_profile_code_from_transfer_syntax_v2(
  ...
  if (ctx->binding_kind == DecoderBindingKind::kCoreDirect) {
    if (options != nullptr && options->items != nullptr && options->count > 0u) {
      return fail_decoder(ctx, PIXEL_CODEC_ERR_UNSUPPORTED, "parse_options",
          "core direct decoder path does not support options");
    }
...
pixel_error_code_v2 configure_host_encoder_context_v2(HostEncoderContextV2* ctx,
  ...
  if (ctx->binding_kind == EncoderBindingKind::kCoreDirect) {
    if (options != nullptr && options->items != nullptr && options->count > 0u) {
      return fail_encoder(ctx, PIXEL_CODEC_ERR_UNSUPPORTED, "parse_options",
          "core direct encoder path does not support options");
    }
    return PIXEL_CODEC_ERR_OK;
  }
...
pixel_error_code_v2 decode_frame_with_host_context_v2(HostDecoderContextV2* ctx,
    ...
    const HostValueTransformSpecV2* value_transform) {
  ...
  if (value_transform != nullptr &&
      value_transform->kind != HostValueTransformKindV2::kNone) {
  ...
  if (value_transform != nullptr) {
    switch (value_transform->kind) {
    case HostValueTransformKindV2::kNone:
      break;
    case HostValueTransformKindV2::kRescale:
      request.value_transform.transform_kind = PIXEL_DECODER_VALUE_TRANSFORM_RESCALE_V2;
      request.value_transform.rescale_slope = value_transform->rescale_slope;
      request.value_transform.rescale_intercept = value_transform->rescale_intercept;
      break;
    case HostValueTransformKindV2::kModalityLut:
      if (value_transform->modality_lut == nullptr ||
          value_transform->modality_lut->values.empty()) {
        return fail_decoder(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "validate",
      ...
      request.value_transform.lut_first_mapped =
          value_transform->modality_lut->first_mapped;
      request.value_transform.lut_value_count =
          static_cast<uint64_t>(value_transform->modality_lut->values.size());
      request.value_transform.lut_values_f32.data =
          reinterpret_cast<const uint8_t*>(
              value_transform->modality_lut->values.data());
      request.value_transform.lut_values_f32.size =
          static_cast<uint64_t>(
              value_transform->modality_lut->values.size() * sizeof(float));
      break;
    }
  }
