  ...
  if (options) {
    ...
    for (std::uint32_t i = 0; i < options->count; ++i) {
      ...
      if (option_key_matches(key, "threads") ||
        ...
        if (!parse_strict_int_option(std::string_view(item.value),
                configured_threads)) {
          set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
              DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
              "JPEG2000 decoder threads option must be an integer");
          return 0;
        }
        threads_seen = true;
        continue;
...
int jpeg2k_decoder_decode_frame(void* ctx,
  ...
  if (!is_jpeg2k_transfer_syntax_code(request->frame.transfer_syntax_code)) {
  ...
  if (!decode_jpeg2k_into(info, value_transform, destination, strides, options,
          decode_error, source)) {
  ...
  if (options) {
    for (std::uint32_t i = 0; i < options->count; ++i) {
      ...
      }
      {
        set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
            DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
            std::string("unknown JPEG2000 option key: ") + std::string(key));
        return 0;
      }
    }
  }
...
int jpeg2k_encoder_encode_frame(void* ctx,
  ...
  if (!try_encode_jpeg2k_frame(
          ...
          use_multicomponent_transform, lossless, context->options,
