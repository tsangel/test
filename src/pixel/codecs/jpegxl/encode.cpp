...
pixel_error_code check_jxl_success(EncoderCtx* ctx, JxlEncoder* encoder,
...
pixel_error_code validate_encoder_request(
  ...
  const bool lossless =
      request->frame.codec_profile_code == PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS;
...
pixel_error_code encoder_encode_frame_to_context_buffer(
  ...
  try {
    ...
    if (worker_threads > 0) {
      ...
      const pixel_error_code ec = check_jxl_success(c, encoder.get(),
