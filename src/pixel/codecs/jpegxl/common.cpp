...
constexpr pixel_supported_profile_flags kSupportedProfileFlags =
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_DEC |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSY_DEC |
    PIXEL_CODEC_PROFILE_JPEGXL_JPEG_RECOMPRESSION_DEC |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS_ENC |
    PIXEL_CODEC_PROFILE_JPEGXL_LOSSY_ENC;
...
pixel_error_code parse_decoder_options(
  ...
  for (uint32_t i = 0; i < options->count; ++i) {
    ...
    if (std::strcmp(kv.key, "threads") == 0 ||
        std::strcmp(kv.key, "decoder_threads") == 0) {
      int parsed = 0;
...
pixel_error_code parse_encoder_options(
  ...
  for (uint32_t i = 0; i < options->count; ++i) {
    ...
    if (std::strcmp(kv.key, "distance") == 0) {
      double parsed = 0.0;
...
pixel_error_code decoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_decoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported decoder codec_profile_code=%u for jpegxl",
        codec_profile_code);
  }
...
pixel_error_code encoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_encoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported encoder codec_profile_code=%u for jpegxl",
        codec_profile_code);
  }

  c->distance =
      (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS) ? 0.0 : 1.0;
  c->effort = 7;
  c->threads = -1;
  ...
  if (c->threads < -1) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
        "threads must be -1, 0, or positive");
  }
  ...
  if (!std::isfinite(c->distance) || c->distance < 0.0 || c->distance > 25.0) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
        "distance must be in [0,25]");
  }

  if (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGXL_LOSSLESS &&
      c->distance != 0.0) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
