...
constexpr pixel_supported_profile_flags kSupportedProfileFlags =
    ...
    PIXEL_CODEC_PROFILE_HTJ2K_LOSSLESS_ENC |
    PIXEL_CODEC_PROFILE_HTJ2K_LOSSLESS_RPCL_ENC |
    PIXEL_CODEC_PROFILE_HTJ2K_LOSSY_ENC;
...
pixel_error_code parse_encoder_options(
  ...
  for (uint32_t i = 0; i < options->count; ++i) {
    ...
    if (std::strcmp(kv.key, "qstep") == 0) {
      double parsed = 0.0;
      if (!parse_double_option(kv.value, &parsed)) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "qstep must be a finite number");
      }
      if (parsed <= 0.0 || parsed > 0.5) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "qstep must be in (0,0.5]");
      }
      ctx->qstep = parsed;
      ctx->has_explicit_qstep = true;
      continue;
    }
    if (std::strcmp(kv.key, "target_bpp") == 0) {
      ...
      if (!parse_double_option(kv.value, &parsed)) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "target_bpp must be a finite number");
      }
      if (parsed < 0.0) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
    ...
    if (std::strcmp(kv.key, "target_psnr") == 0) {
      ...
      if (!parse_double_option(kv.value, &parsed)) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "target_psnr must be a finite number");
      }
      if (parsed < 0.0) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
...
pixel_error_code decoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_decoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported decoder codec_profile_code=%u for htj2k",
        codec_profile_code);
  }
...
pixel_error_code encoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_encoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported encoder codec_profile_code=%u for htj2k",
        codec_profile_code);
  }
