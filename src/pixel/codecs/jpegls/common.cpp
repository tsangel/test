...
template <typename Ctx>
pixel_error_code fail_detail_impl(
    Ctx* ctx, pixel_error_code code, const char* stage, const char* reason) {
...
template <typename Ctx>
pixel_error_code fail_detail_u32_impl(Ctx* ctx, pixel_error_code code,
...
constexpr pixel_supported_profile_flags kSupportedProfileFlags =
    PIXEL_CODEC_PROFILE_JPEGLS_LOSSLESS_DEC |
    PIXEL_CODEC_PROFILE_JPEGLS_NEAR_LOSSLESS_DEC |
    PIXEL_CODEC_PROFILE_JPEGLS_LOSSLESS_ENC |
    PIXEL_CODEC_PROFILE_JPEGLS_NEAR_LOSSLESS_ENC;
...
pixel_error_code parse_decoder_options(
  ...
  for (uint32_t i = 0; i < options->count; ++i) {
    const pixel_option_kv& kv = options->items[i];
    ...
    std::snprintf(reason, sizeof(reason),
        "JPEG-LS decoder does not accept option '%s'", kv.key);
...
pixel_error_code parse_encoder_options(
  ...
  for (uint32_t i = 0; i < options->count; ++i) {
    ...
    if (std::strcmp(kv.key, "near_lossless_error") == 0) {
      int parsed = 0;
      if (!parse_int_option(kv.value, &parsed)) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "near_lossless_error must be an integer");
      }
      if (parsed < 0 || parsed > 255) {
        return fail_detail(ctx, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "parse_options",
            "near_lossless_error must be in [0,255]");
      }
      ctx->near_lossless_error = parsed;
      continue;
    }
...
pixel_error_code decoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_decoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported decoder codec_profile_code=%u for jpegls",
        codec_profile_code);
  }

  const pixel_error_code ec = parse_decoder_options(c, options);
...
pixel_error_code encoder_configure(void* ctx, uint32_t codec_profile_code,
  ...
  if (!is_supported_encoder_profile(codec_profile_code)) {
    return fail_detail_u32(c, PIXEL_CODEC_ERR_UNSUPPORTED, "configure",
        "unsupported encoder codec_profile_code=%u for jpegls",
        codec_profile_code);
  }

  c->near_lossless_error =
      (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGLS_LOSSLESS) ? 0 : 1;
  ...
  if (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGLS_LOSSLESS &&
      c->near_lossless_error != 0) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "configure",
        "lossless JPEG-LS requires near_lossless_error=0");
  }

  if (codec_profile_code == PIXEL_CODEC_PROFILE_JPEGLS_NEAR_LOSSLESS &&
      c->near_lossless_error <= 0) {
    return fail_detail(c, PIXEL_CODEC_ERR_INVALID_ARGUMENT, "configure",
        "near-lossless JPEG-LS requires near_lossless_error>0");
  }
