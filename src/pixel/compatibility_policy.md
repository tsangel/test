...
- 대상: `src/pixel` 하위의 ABI/runtime/codec/plugin 문서 및 구현
- 버전: v2 (clean-slate)

## 2. Terminology Rules

1. `codec`는 pixel 압축/비압축 family, profile, 옵션, 에러, 구현 코드를 가리키는 기본 용어로 사용한다.
2. `plugin`은 runtime registry가 소비하는 ABI binding 또는 loadable/shared packaging 단위를 가리킬 때만 사용한다.
3. 모든 plugin은 어떤 codec family를 제공하지만, 모든 codec이 plugin은 아니다.
4. `NATIVE_UNCOMPRESSED`와 `ENCAPSULATED_UNCOMPRESSED`는 codec이지만 plugin이 아닌 core direct route다.
5. `RLE_LOSSLESS`와 압축 codec family는 builtin plugin binding 또는 loadable plugin export를 가질 수 있다.
6. `codec_profile_code`, `codec option`, `codec error`처럼 알고리즘/프로파일 의미가 중심일 때는 `codec`을 쓴다.
7. `plugin_api`, `plugin loader`, `plugin registration`, `builtin plugin`, `external plugin`처럼 ABI/package 의미가 중심일 때는 `plugin`을 쓴다.
...
## 7. Codec Placement Policy

1. `NATIVE_UNCOMPRESSED`와 `ENCAPSULATED_UNCOMPRESSED`는 plugin 경계를 통과하지 않고 `pixel_` core 경로에서 직접 처리한다.
2. `RLE_LOSSLESS`는 `src/pixel/codecs/rle_v2`에 builtin codec implementation + builtin plugin binding으로 둔다.
3. JPEG/JPEG-LS/JPEG2000/HTJ2K/JPEG-XL 계열의 코드 위치는 `src/pixel/codecs/*_v2`를 기준으로 한다.
4. codec family별로 builtin plugin binding과 loadable plugin export를 같은 디렉터리에서 병행 구현할 수 있다.
5. 구조 통일을 위해 코드 루트는 모두 `src/pixel` 아래에 유지한다.

## 8. Registry Policy (v2)

1. registry는 `profile_code` 기준 고정 32슬롯 테이블(`0..31`)을 사용한다.
2. 각 슬롯은 decoder/encoder 엔트리를 함께 보관한다.
3. plugin registration 경로의 슬롯 채움은 plugin API가 제공하는 `supported_profile_flags`(64-bit)를 decode해서 수행한다.
4. 동일 슬롯 충돌은 decoder/encoder 각각 `last registration wins` 정책으로 처리한다.
5. 선택 기준으로 `priority`나 별도 plugin 식별자를 사용하지 않는다.

## 9. Runtime Bootstrap Policy

1. `initialize_registry_v2`는 같은 `BindingRegistryRuntimeV2` 인스턴스에서 `std::call_once`로 1회만 실제 초기화를 수행한다.
2. `shutdown_registry_v2` 호출 이후에는 동일 인스턴스 재초기화를 허용하지 않는다.
3. `init_builtin_registry_v2`는 core direct routes + static-linked builtin plugin bindings를 등록한다.
