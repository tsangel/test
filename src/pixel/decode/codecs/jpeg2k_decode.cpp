...
OPJ_UINT32 resolve_openjpeg_thread_count(const DecodeOptions& opt) {
	...
	if (static_cast<unsigned long long>(configured_threads) >
		...
		throw_decode_error(
		    "decoder_threads {} exceeds OpenJPEG limit {}",
...
opj_image_ptr decode_openjpeg_image_with_format(
	...
	const auto thread_count = resolve_openjpeg_thread_count(opt);
...
opj_image_ptr decode_openjpeg_image(
	...
	if (auto image = decode_openjpeg_image_with_format(
	        source, first_format, opt, first_failure)) {
		return image;
	}
	if (auto image = decode_openjpeg_image_with_format(
	        source, second_format, opt, second_failure)) {
		return image;
	}
...
void validate_decoded_image(const pixel::PixelDataInfo& info,
	...
	if (decoded_rows != rows || decoded_cols != cols) {
		throw_decode_error(
		    "JPEG2000 decoded dimensions mismatch (decoded={}x{}, expected={}x{})",
		    decoded_rows, decoded_cols, rows, cols);
	}
	if (image.numcomps != samples_per_pixel) {
		throw_decode_error(
		    "JPEG2000 component count mismatch (decoded={}, expected={})",
	...
	if (!expected_signed && max_precision == 32) {
		throw_decode_error(
		    "JPEG2000 unsigned 32-bit samples are not supported");
	}
	...
	for (std::size_t c = 0; c < samples_per_pixel; ++c) {
		...
		if (!comp.data) {
			throw_decode_error(
			    "JPEG2000 component {} has no decoded data",
			    c);
		}
		if (comp.w != cols || comp.h != rows) {
			throw_decode_error(
			    "JPEG2000 component {} dimensions mismatch (decoded={}x{}, expected={}x{})",
			    c, comp.h, comp.w, rows, cols);
		}
		if (comp.sgnd != static_cast<OPJ_UINT32>(expected_signed ? 1 : 0)) {
			throw_decode_error(
			    "JPEG2000 component {} signedness mismatch (decoded={}, expected={})",
			    c, comp.sgnd, expected_signed ? 1 : 0);
		}
		if (comp.prec == 0 || comp.prec > max_precision) {
			throw_decode_error(
			    "JPEG2000 component {} precision {} exceeds output {} bits",
			    c, comp.prec, max_precision);
		}
	}
}
