...
int jpegxl_decoder_decode_frame(void* ctx,
  ...
  if (!is_jpegxl_transfer_syntax_code(request->frame.transfer_syntax_code)) {
  ...
  if (!decode_jpegxl_into(info, value_transform, destination, strides, options,
          decode_error, source)) {
...
int jpegxl_encoder_configure(void* ctx, std::uint16_t transfer_syntax_code,
  ...
  if (options) {
    for (std::uint32_t i = 0; i < options->count; ++i) {
      ...
      if (option_key_matches(key, "distance")) {
        ...
        configured_options.distance = parsed;
        continue;
      }
      if (option_key_matches(key, "effort")) {
        ...
        configured_options.effort = parsed;
        continue;
      }
  ...
  if (configured_options.effort < 1 || configured_options.effort > 10) {
    set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
        DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
        "JpegXlOptions.effort must be in [1,10]");
    return 0;
  }
  if (!std::isfinite(configured_options.distance) ||
      configured_options.distance < 0.0 || configured_options.distance > 25.0) {
    set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
        DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
        "JpegXlOptions.distance must be in [0,25]");
    return 0;
  }
  if (transfer_syntax->is_lossless() && configured_options.distance != 0.0) {
    set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
        DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
        "JPEGXLLossless transfer syntax requires distance=0");
    return 0;
  }
  if (transfer_syntax->is_lossy() && configured_options.distance <= 0.0) {
    set_abi_error(error, DICOMSDL_CODEC_INVALID_ARGUMENT,
        DICOMSDL_CODEC_STAGE_PARSE_OPTIONS,
