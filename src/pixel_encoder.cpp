...
void update_pixel_metadata_for_set_pixel_data(DataSet& dataset, std::string_view file_path,
    ...
    int bits_allocated, int bits_stored, int high_bit, int pixel_representation,
	...
	const auto photometric_text = to_photometric_text(source.photometric);
...
void DicomFile::set_pixel_data(uid::WellKnown transfer_syntax, const pixel::PixelSource& source,
	...
	const bool target_is_j2k =
	    transfer_syntax.supports_pixel_encode() &&
	    transfer_syntax.is_jpeg2000() &&
	    !transfer_syntax.is_htj2k();
	const bool target_is_j2k_lossless = target_is_j2k && transfer_syntax.is_lossless();
	const bool target_is_j2k_lossy = target_is_j2k && transfer_syntax.is_lossy();
	const bool target_is_rle = transfer_syntax.is_rle();
	const bool target_is_native_uncompressed =
	    transfer_syntax.is_uncompressed() && !transfer_syntax.is_encapsulated();
	if (!transfer_syntax.supports_pixel_encode()) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} ts={} reason=transfer syntax is not supported yet in set_pixel_data (supported: native uncompressed, RLELossless, JPEG2000*)",
		    path(), transfer_syntax.value());
	}
	auto resolved_codec_opt = codec_opt;
	if (std::holds_alternative<pixel::AutoCodecOptions>(resolved_codec_opt)) {
		if (target_is_j2k) {
			pixel::J2kOptions default_j2k{};
			if (target_is_j2k_lossy) {
				default_j2k.target_psnr = kDefaultLossyJ2kTargetPsnr;
			}
			resolved_codec_opt = default_j2k;
		} else if (target_is_rle) {
			resolved_codec_opt = pixel::RleOptions{};
		} else {
			resolved_codec_opt = pixel::NoCompression{};
		}
	}
	if (target_is_j2k) {
		if (!std::holds_alternative<pixel::J2kOptions>(resolved_codec_opt)) {
			diag::error_and_throw(
			    "DicomFile::set_pixel_data file={} ts={} reason=JPEG2000 transfer syntax requires J2kOptions codec",
			    path(), transfer_syntax.value());
		}
	} else if (target_is_rle) {
		if (!std::holds_alternative<pixel::RleOptions>(resolved_codec_opt)) {
			diag::error_and_throw(
			    "DicomFile::set_pixel_data file={} ts={} reason=RLELossless transfer syntax requires RleOptions codec",
			    path(), transfer_syntax.value());
		}
	} else if (!std::holds_alternative<pixel::NoCompression>(resolved_codec_opt)) {
		diag::error_and_throw(
	...
	int bits_allocated = source.bits_allocated.value_or(native_layout->bits_allocated);
	int bits_stored = source.bits_stored.value_or(bits_allocated);
	int high_bit = source.high_bit.value_or(bits_stored - 1);
	int pixel_representation =
	    source.pixel_representation.value_or(native_layout->pixel_representation);

	if (bits_allocated != native_layout->bits_allocated) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} reason=bits_allocated({}) must match data_type storage width({})",
		    path(), bits_allocated, native_layout->bits_allocated);
	}
	if (pixel_representation != native_layout->pixel_representation) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} reason=pixel_representation({}) must match data_type signedness({})",
		    path(), pixel_representation, native_layout->pixel_representation);
	}
	...
	if (high_bit < 0 || high_bit >= bits_allocated) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} reason=high_bit({}) must be in [0, bits_allocated({}) - 1]",
		    path(), high_bit, bits_allocated);
	}
	if (pixel_representation != 0 && pixel_representation != 1) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} reason=pixel_representation must be 0 or 1",
		    path());
	}
	if (target_is_j2k && bits_allocated > 16) {
		diag::error_and_throw(
		    "DicomFile::set_pixel_data file={} reason=JPEG2000 encoder currently supports bits_allocated <= 16",
		    path());
	}

	root_dataset_->ensure_loaded(Tag(0xFFFFu, 0xFFFFu));
	update_pixel_metadata_for_set_pixel_data(
	    *root_dataset_, path(), transfer_syntax, source, target_is_rle,
	...
	if (target_is_j2k) {
		const bool use_mct =
		    transfer_syntax == "JPEG2000MCLossless"_uid ||
		    transfer_syntax == "JPEG2000MC"_uid;
		if (use_mct && samples_per_pixel != std::size_t{3}) {
			diag::error_and_throw(
			    "DicomFile::set_pixel_data file={} ts={} reason=JPEG2000 MC transfer syntax requires samples_per_pixel=3",
			    path(), transfer_syntax.value());
		}

		const auto& j2k_options = std::get<pixel::J2kOptions>(resolved_codec_opt);
		pixel::detail::encode_jpeg2k_pixel_data(
		    *this, encapsulated_encode_input, rows, cols, samples_per_pixel,
		    bytes_per_sample, bits_allocated, bits_stored, pixel_representation,
		    source.planar, source_row_stride, use_mct, target_is_j2k_lossless,
		    j2k_options, path());
	} else if (target_is_rle) {
		pixel::detail::encode_rle_pixel_data(
		    *this, encapsulated_encode_input, rows, cols, samples_per_pixel,
		    bytes_per_sample, source.planar, source_row_stride, path());
	} else {
