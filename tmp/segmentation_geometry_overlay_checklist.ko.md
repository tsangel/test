...
- [x] BINARY 1-bit frame read source API 구현: `BinaryFrameBitsView`와 `for_each_binary_frame_set_bit()`로 native PixelData와 Encapsulated Uncompressed frame payload를 `uint8` frame으로 전부 unpack하지 않고 set-bit iterator로 처리할 수 있게 했다. 최소 byte window, unaligned native frame start, DICOM LSB-first bit order, even-length zero padding, detached/missing/true-compressed reject, non-BINARY reject, visitor exception propagation을 smoke test로 고정했다.
- [x] DicomSDL BINARY label volume core 구현: `BinaryLabelCodeTable`, overlap `label_code -> label_set` table, `build_binary_label_volume_into()` caller-owned `std::span<uint16_t>` target, owning convenience `BinaryLabelVolume`, `BinarySegmentationFrameSetBitSource`, `restore_binary_label_mask()`를 추가했다. BINARY label volume 전용 public API는 `dicom_seg_binary_label_volume.h`, 구현은 `segmentation_binary_label_volume.cpp`로 분리해 `dicom_seg.h`가 SEG reader/frame API 중심으로 남도록 했다. grid/placement/resampling과 GPU texture/LUT는 caller/viewer 책임으로 남기고, DicomSDL은 BINARY SEG set-bit source를 packed label volume으로 변환하는 공통 core까지만 제공한다.
- [x] Python BINARY label volume wrapper 구현: `Segmentation.build_binary_label_volume(frame_placements, slices=...)`가 DicomSDL core builder를 호출해 `dicom.seg.BinaryLabelVolume`을 반환한다. `Segmentation.build_binary_label_volume_into(out, frame_placements, slices=...)`는 caller-owned native `uint16` buffer를 직접 채우고 같은 metadata 객체를 반환한다. 결과 객체는 read-only `uint16` `label_volume` NumPy view, `source_dicom_segment_by_label_id`, `label_set()`, `label_codes_for_label_id()`, `restore_mask_for_label_id()`, `restore_mask_for_segment()`를 제공한다.
...
- [x] SEG Encapsulated write path의 Extended Offset Table coverage 추가: LABELMAP 2-frame SEG를 Encapsulated Uncompressed/RLE로 쓴 뒤 `PixelSequence::extended_offset_table_count()`와 SEG decode 결과를 검증한다.
- [ ] 남은 후속: Python fake shared plugin fixture 기반 unknown lossy-state reject 테스트, 실제 외부 compressed FRACTIONAL/LABELMAP sample 기반 regression, true-compressed BINARY 1-bit core layout/write 지원을 진행한다.
...
- [ ] true-compressed BINARY SEG는 기존 `PixelPayloadDecoder`가 `BitsAllocated` 8/16/32만 받는 한 바로 지원하지 않는다. Encapsulated Uncompressed BINARY read/decode는 `BinaryFrameBitsView`로 지원하지만, RLE/JPEG 계열 compressed BINARY는 구현 전 `BitsAllocated=1` decoded layout, unpacked bool/u8 view, native 1-bit repack, encapsulated frame encoding contract 중 어떤 계층에서 1-bit를 표현할지 먼저 확정한다.
...
- [x] Extended Offset Table/Extended Offset Table Lengths가 있는 파일은 기존 payload splitter/write path가 지원하는 범위 안에서 허용한다. SEG writer smoke는 Encapsulated Uncompressed/RLE roundtrip에서 EOT backpatch와 frame decode를 검증한다. 부족한 부분이 발견되면 generic PixelSequence/payload indexing 개선 항목으로 처리한다.
