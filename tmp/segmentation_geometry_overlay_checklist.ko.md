...
- [ ] 입력 decode는 `DecodeInfo::encoded_lossy_state` 또는 동등한 codec result 상태로 확인해 `lossy`, `near_lossless`, `unknown`을 cache publish 전에 reject한다. 1차로 SEG public decode path의 FRACTIONAL/LABELMAP frame decode는 `DecodeInfo`를 확인해 lossless source만 허용한다. write/transcode streaming input decode path는 별도 후속으로 남긴다.
...
- [ ] write/transcode 중 semantic validation 범위를 고정한다. LABELMAP public decode path는 decoded label membership, `PixelPaddingValue` background 제외, 16-bit host/native-endian label contract를 검증한다. LABELMAP write/transcode streaming frame scan과 FRACTIONAL sample `MaximumFractionalValue` 초과 reject 여부는 구현 전에 결정한다.
...
- [x] write/transcode 후 validation test를 추가한다. Encapsulated Uncompressed output을 다시 SEG로 열어 C++ `decode_frame_into`/`decode_labelmap_frame_into`, Python `to_array`, `present_segment_numbers`, `mask_for_segment`, `frames_for_segment`, `validate_label_values`를 실행하고 원본 frame별 mask/label value와 비교한다. RLE/JPEG-LS/JPEG 2000 등 codec-specific lossless compressed regression은 별도 항목으로 남긴다.
